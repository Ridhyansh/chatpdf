...
from services.auth_service import UserCreate, UserLogin, signup as auth_signup, login as auth_login, decode_token
...
async def auth_signup_endpoint(payload: UserCreate):
    try:
...
@app.post("/auth/login")
async def auth_login_endpoint(payload: UserLogin):
    try:
...
origins = [
    ...
    "http://localhost:3001",  # Alternate dev port if 3000 occupied
...
async def get_pdf(filename: str, request: Request):
    try:
        ...
        try:
            public_base = os.getenv('PUBLIC_BACKEND_BASE', 'http://localhost:8000').rstrip('/')
            file_url = f"{public_base}/pdf/{filename}"
        ...
        # Determine allowed origin dynamically (prefer request origin if recognized or localhost)
        req_origin = request.headers.get("origin")
        if req_origin and (req_origin in origins or req_origin.startswith("http://localhost")):
            allow_origin = req_origin
        else:
            # Fallback to production domain
            allow_origin = "https://nexus-ai-lac.vercel.app"
...
async def upload_pdf(file: UploadFile = File(...), user: str = Depends(get_current_user)):
    try:
        ...
        try:
            ...
            public_base = os.getenv('PUBLIC_BACKEND_BASE', 'http://localhost:8000').rstrip('/')
            file_url = f"{public_base}/pdf/{safe_filename}"
            await chat_service.extract_text_from_pdf(file_url)
        ...
        return {
            ...
            "url": file_url,  # absolute (public) URL
            "relative_url": f"/pdf/{safe_filename}",  # for local development same-origin access
            "status": "success"
        }
    except Exception as e:
...
@app.post("/compare")
async def compare_pdfs(original: UploadFile = File(...), compare: UploadFile = File(...), user: str = Depends(get_current_user)):
    try:
